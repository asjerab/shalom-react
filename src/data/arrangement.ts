export const arrangement = [
  {
    id: "Storsondag",
    title: "Storsøndag",
    description:
      "Storsøndag sammen med filadelfia kirken. Vi oppfordrer alle til å delta på dette møtet.",
    location: "St. Olavs Gate 24, Oslo",
    clock: "Tid etter avtale",
    date: "15.09.24",
  },

  {
    id: "JulebordYouth",
    title: "Julebord for youth connect",
    description:
      "Julebord for de som er i connect Youth, mer info innad gruppene deres.",
    location: "St. Olavs Gate 24, Oslo",
    clock: "Tid etter avtale",
    date: "11.12.24",
  },

  {
    id: "Julecup",
    title: "Julecup  Youth & New Creation",
    description: "Mer info kommer.",
    location: "lokasjon kommer",
    clock: "Tid etter avtale",
    date: "21.12.24",
  },

  {
    id: "Bravelovenorway",
    title: "Jente arrangement med Bravelovenorway",
    description:
      "5. desember kommer Brave Love til Oslo for å ha et event med Shalom Youth og Shalom New Creation! Hold av datoen og kom!",
    location: "Pilstredet 27H, Oslo",
    clock: "18:00",
    date: "05.12.24",
  },

  {
    id: "Nyttaarsaften",
    title: "Nyttårsaften feiring",
    description:
      "Bli med på en uforglemmelig nyttårsfeiring sammen med Shalom Youth og Shalom New Creation! 🎉🎊 All nødvendig informasjon og lenken til påmelding finner du nedenfor. Vi gleder oss til å se deg der! https://docs.google.com/forms/d/e/1FAIpQLSftAt-iPXwFYcB77a6KTdZI8IgLYGqlSj0P2cY4GRyL5HlsVg/viewform",
    location: "St. Olavs Gate 24, Oslo",
    clock: "20:00 - 00:15",
    date: "31.12.24",
  },
  {
    id: "Paaskekonf",
    title: "Påskekonferanse",
    description:
      "Må han som er så rik på herlegdom, styrkja dykkar indre menneske med kraft ved sin Ande. / Gjestetaler : Pastor Albert",
    location: "St. Olavs Gate 24, Oslo",
    clock: "18:00",
    date: "11.07",
    //
  },
  {
    id: "PaaskekonfDagTo",
    title: "Påskekonferanse dag to",
    description:
      "Må han som er så rik på herlegdom, styrkja dykkar indre menneske med kraft ved sin Ande. / Gjestetaler : Pastor Albert",
    location: "St. Olavs Gate 24, Oslo",
    clock: "16:00",
    date: "12.07",
    //
  },
  {
    id: "ShalomYouthLeir",
    title: "Shalom Youth sommerleir",
    description:
      "Shalom youth's årlige sommerleir. YOU DONT WANNA MISS OUT! Lenke til påmelding finner du nede. Alt av info befinner seg på lenken har du noen spørsmål kan du ta kontakt med oss på mail som du finner på kontakt siden. Ta med en venn eller ti å opplev hva Gud har i vente for oss denne sommeren! https://event.checkin.no/113512/shalom-youth-sommerleir-2025",
    location: "Fyresdal leirsted",
    clock: "Klokkslett finner du i lenken",
    date: "28.07 - 31.07",
    //
  },

  {
    id: "JesusISentrum",
    title: "Jesus i Sentrum",
    description:
      "Denne lørdagen delatar vi på Jesus i sentrum sammen med andre felleskap. Blir ikke noe gudstjeneste i dag mer info på instagram https://www.instagram.com/jesusisentrum/",
    location: "Spikersuppa i Oslo",
    clock: "Følg meg på insta",
    date: "03.05",
    //
  },

  {
    id: "LovsangsKveld",
    title: "Lovsangskveld",
    description:
      "Bli med å delta på en kveld hvor vi løfter navnet Jesus sammen i lovsang som søsken i kristus.",
    location: "St. Olavs gate 24",
    clock: "18:00",
    date: "18.05",
    //
  },
];
